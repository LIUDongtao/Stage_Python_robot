...
class YoloSemanticDbscanForced(Node):
    def __init__(self, args):
        ...
        # Raw YOLO points are temporary. If an object is not detected again
        # within point_ttl seconds, it is removed from the semantic map.
        self.point_ttl = float(args.point_ttl)
        self.marker_lifetime = float(args.marker_lifetime)
        self.tf_timeout = float(args.tf_timeout)
        self.tf_debug = bool(args.tf_debug)
        self.use_latest_tf_fallback = bool(args.use_latest_tf_fallback)
        self._last_tf_debug_sec = 0.0
        if self.marker_lifetime <= 0.0 and self.point_ttl > 0.0:
            self.marker_lifetime = self.point_ttl
        ...
        self.get_logger().info(f"Point TTL: {self.point_ttl} s")
        self.get_logger().info(f"TF mode: image timestamp first, latest fallback={self.use_latest_tf_fallback}")
        if DBSCAN is None:
    ...
    def transform_camera_to_target(self, x, y, z, stamp_msg=None):
        # IMPORTANT:
        # Try the RGB image timestamp first. With ZED + RTAB-Map, TF can lag behind
        # the image stamp by 1-2 seconds, so we fall back to the latest TF by default.
        # This avoids using an initial/stale pose while keeping detections alive.
        if stamp_msg is None:
            tf_time = rclpy.time.Time()
            stamp_text = 'latest'
        else:
            tf_time = rclpy.time.Time.from_msg(stamp_msg)
            stamp_text = f'{stamp_msg.sec}.{stamp_msg.nanosec:09d}'
        ...
        try:
            ...
            if not self.use_latest_tf_fallback:
                self.get_logger().warn(
                    f"TF at image time not ready and latest fallback disabled: {self.target_frame} <- {self.camera_frame}, "
                    f"image_stamp={stamp_text}: {e}",
            ...
            if self.tf_debug:
                self.get_logger().warn(
                    f"TF at image time not ready, using latest TF fallback: {self.target_frame} <- {self.camera_frame}, "
                    f"image_stamp={stamp_text}: {e}",
                    throttle_duration_sec=2.0
                )
            ...
            try:
                ...
                self.get_logger().warn(
                    f"TF not ready: {self.target_frame} <- {self.camera_frame}: {e2}",
                    throttle_duration_sec=2.0
                )
                return None
        ...
        now = self.now_sec()
        if self.tf_debug and now - self._last_tf_debug_sec > 1.0:
            self._last_tf_debug_sec = now
            mode = 'latest_fallback' if used_latest_fallback else 'image_stamp'
            self.get_logger().info(
                f"TF[{mode}] {self.target_frame} <- {self.camera_frame}: "
                f"xyz=({t.x:.3f}, {t.y:.3f}, {t.z:.3f}), "
                f"quat=({r.x:.3f}, {r.y:.3f}, {r.z:.3f}, {r.w:.3f}), "
                f"image_stamp={stamp_text}"
            )
    ...
    def prune_old_points(self):
        if self.point_ttl <= 0.0:
            return

        cutoff = self.now_sec() - self.point_ttl
        empty_names = []
        for name, points in self.raw_points.items():
            self.raw_points[name] = [p for p in points if p[3] >= cutoff]
            if not self.raw_points[name]:
                empty_names.append(name)

        for name in empty_names:
            del self.raw_points[name]

    def add_raw_point(self, name, pos):
        points = self.raw_points[name]
        points.append((float(pos[0]), float(pos[1]), float(pos[2]), self.now_sec()))
    ...
    def image_callback(self, msg: Image):
        ...
        for result in results:
            ...
            for box in result.boxes:
                ...
                map_pos = self.transform_camera_to_target(cam_x, cam_y, cam_z, msg.header.stamp)
                if map_pos is None:
                    continue
    ...
    def cluster_points(self, points):
        ...
        X = np.array([(p[0], p[1], p[2]) for p in points], dtype=np.float32)
        X2 = X[:, :2]
...
def main():
    ...
    parser.add_argument('--point_ttl', type=float, default=5.0,
                        help='Seconds to keep a detection if it is not seen again. 0 disables deletion.')
    parser.add_argument('--marker_lifetime', type=float, default=0.0,
                        help='RViz marker lifetime. 0 means use point_ttl.')
    parser.add_argument('--text_scale', type=float, default=0.25)
    parser.add_argument('--tf_timeout', type=float, default=0.2,
                        help='Seconds to wait for TF at the RGB image timestamp.')
    parser.add_argument('--tf_debug', action='store_true',
                        help='Print the TF translation/rotation used for YOLO projection once per second.')
    parser.add_argument('--use_latest_tf_fallback', action='store_true', default=True,
                        help='If TF at the image timestamp is unavailable, fall back to latest TF. Enabled by default for ZED + RTAB-Map timestamp lag.')
    parser.add_argument('--no_latest_tf_fallback', dest='use_latest_tf_fallback', action='store_false',
                        help='Disable latest-TF fallback and only accept TF exactly at the RGB image timestamp.')
